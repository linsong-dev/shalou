...
Write-Host "  Shalou · 流动的记忆 部署" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan

$srcRoot = Resolve-Path (Join-Path $PSScriptRoot "..")
$agentsDir = "$env:USERPROFILE\.agents"
$pluginMarketDir = "$agentsDir\plugins\shalou"
...
if (-not (Test-Path $mktplFile)) {
    ...
    $m = @{ name="personal"; interface=@{displayName="Personal"}; plugins=@(
        ...
        @{ name="shalou"; source=@{source="local"; path="./plugins/shalou"}; policy=@{installation="AVAILABLE";authentication="ON_INSTALL"}; category="Productivity" }
    )}
    Write-NoBOMJson -Path $mktplFile -Object $m
} else {
    $m = Get-Content $mktplFile -Encoding UTF8 -Raw | ConvertFrom-Json
    if ($m.plugins.name -notcontains "shalou") {
        $m.plugins += @{ name="shalou"; source=@{source="local"; path="./plugins/shalou"}; policy=@{installation="AVAILABLE";authentication="ON_INSTALL"}; category="Productivity" }
        Write-NoBOMJson -Path $mktplFile -Object $m
    }
}
...
if ($cli) {
    & $cli plugin remove "shalou@personal" 2>&1 | Out-Null
    & $cli plugin add "shalou@personal" 2>&1 | Out-Null
    Write-Host "  shalou@personal 安装完成" -ForegroundColor Green
}
